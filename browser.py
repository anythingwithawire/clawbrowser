...
class BrowserSignals(QObject):
    ...
    resize_sig      = pyqtSignal(str, int, int)  # req_id, width, height
    zoom_sig        = pyqtSignal(str, float)     # req_id, factor
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/navigate":
            ...
            h = int(body.get("height", 900))
            req_id = str(uuid.uuid4())
            signals.resize_sig.emit(req_id, w, h)
            result = wait_result(req_id)
            self._send_json(result)
            ...
            req_id = str(uuid.uuid4())
            signals.zoom_sig.emit(req_id, factor)
            result = wait_result(req_id)
            self._send_json(result)
...
class BrowserWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        signals.resize_sig.connect(self._handle_resize)
    ...
    def _handle_resize(self, req_id: str, width: int, height: int):
        self.resize(width, height)
        set_result(req_id, {"ok": True, "width": width, "height": height})

    def _handle_zoom(self, req_id: str, factor: float):
        view = self.current_view()
        if view:
            view.setZoomFactor(factor)
            set_result(req_id, {"ok": True, "zoom": factor})
        else:
            set_result(req_id, {"error": "no active tab"})
...
def main():
    ...
    parser.add_argument("--virtual", metavar="WxH", default=None,
                        help="Launch on a private Xvfb display at WxH, e.g. --virtual 1920x1080")
    ...
    if args.virtual:
        ...
        m = re.fullmatch(r"(\d+)x(\d+)", args.virtual)
        if not m:
            print(f"ERROR: --virtual must be WxH e.g. 1920x1080", file=sys.stderr)
            sys.exit(1)
        ...
        _xvfb_proc = subprocess.Popen(
            ["Xvfb", display, "-screen", "0", f"{vw}x{vh}x24"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        time.sleep(0.8)
        os.environ["DISPLAY"] = display
        print(f"[ClawBrowser] Virtual display {display} @ {vw}x{vh}")
